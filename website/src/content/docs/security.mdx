...
<Aside type="caution">
  MCP client config files (`claude_desktop_config.json`, `.mcp.json`) may contain credentials. Do not commit them to version control. If you use a shared machine, prefer the `.env` file approach and restrict its permissions: `chmod 600 .env`.
</Aside>
